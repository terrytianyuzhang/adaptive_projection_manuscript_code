...
type_1_error_and_power_plot_output_file <- file.path(
  ...
  "data",
  "final",
  "037_unstructured_rejection_rate.pdf"
)
...
rejection_rate_summary <- rejection_rate_summary[
  rejection_rate_summary$method != "Oracle PC",
  ,
  drop = FALSE
]
rejection_rate_summary$method <- factor(
  rejection_rate_summary$method,
  levels = c("Debiased PC", "Plug-in PC")
)
...
type_1_error_and_power_plot |> print() 
