...
jinhong_folder <- file.path(here(), 
                            "code_submitted", "jinhong_deviance")

source(file.path(here(),"code_submitted", "code_paper", "v2high_dimensional_mean_testing_function.R"))
source(file.path(here(),"code_submitted", "code_paper", "supporting_function.R"))
...
upset_pre_name <- c("All" , "PC1 " , 'glm', "Lasso")
  ...
  for(cell_type in cell_types){
        ...
        result_file_name <- file.path(jinhong_folder, 'result', 
                                      paste0(cell_type, '_cell_version_2.rds'))
        ...
        ###
        result_file_name <- file.path(jinhong_folder, 'result', 
                                      paste0(cell_type, '_cell_version_1.rds'))
        ...
        nb_glm_file <-  file.path(jinhong_folder, 'data',
                                  'pairwise_test', paste0('test_nb_', cell_type,'.csv'))
...
with(listInput,
     intersect(`PC4(p=0.004)`, union(GLM_NB, GLM_NB_ADJ)))
