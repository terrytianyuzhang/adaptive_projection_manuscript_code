...
estimate_nuisance_pc <- function(nuisance_sample_1,
  ...
  if(pca_method == "sparse_pca"){
    ...
    estimate_leading_pc <- dense_leading_pc <- irlba(sample_centered, nv = num_latent_factor)$v
...
evaluate_influence_function <- function(cross_fitting_sample_1,
                                        cross_fitting_sample_2 = NULL,
                                        nuisance_collection){
  ...
  if(is.null(cross_fitting_sample_2)){ # if this is a one-sample testing problem
    ...
    estimate_leading_pc <- nuisance_collection$estimate_leading_pc
    # sigma_times_pc <- nuisance_collection$sigma_times_pc
    #####inner product statistics
    ...
    influence_each_subject_1 <- inner_product_1 + influence_function_eigenvector - as.numeric(ommited_component)
    influence_each_subject_2 <- NULL
  ...
  if(!is.null(cross_fitting_sample_2)){
    ...
    return(list(inner_product_1 = inner_product_1,
                ...
                influence_eigenvector_each_subject_1 = influence_eigenvector_each_subject_1,
                influence_eigenvector_each_subject_2 = influence_eigenvector_each_subject_2,
                for_variance_subject_1 = for_variance_subject_1,
                for_variance_subject_2 = for_variance_subject_2))
    # mu_times_centered <- centered_sample_1 %*% estimate_mean_1
    # mu_times_pc <- as.numeric(crossprod(estimate_leading_pc, estimate_mean_1))
    # 
    # influence_function_eigenvector <- (nuisance_collection$estimate_eigenvalue - nuisance_collection$estimate_noise_variance)^(-1) * (mu_times_centered - mu_times_pc * inner_product_centered_1)*inner_product_centered_1
    ...
    ####influence function of the eigenvector
    # 
    # mu_times_centered <- centered_sample_2 %*% estimate_mean_2
    # mu_times_pc <- as.numeric(crossprod(estimate_leading_pc, estimate_mean_2))
    # 
    # influence_function_eigenvector <- (nuisance_collection$estimate_eigenvalue - nuisance_collection$estimate_noise_variance)^(-1) * (mu_times_centered - mu_times_pc * inner_product_centered_2)*inner_product_centered_2
    # 
    # influence_each_subject_2 <- inner_product_2 + influence_function_eigenvector
  }
...
debiased_cross_fitting_pca <- function(sample_1,
                                       sample_2 = NULL,
                                       pca_method = "sparse_pca",
                                       mean_method = "naive"){
  set.seed(1)
  ...
  crossffiting_two_fold <- vector("list", 2)
  ...
  for(split_index in 1:2){
    crossffiting_two_fold[[split_index]]$nuisance_collection <- estimate_nuisance_parameter(crossffiting_two_fold[[split_index]]$sample_1_nuisance, 
                                                                                            ...
                                                                                            pca_method = pca_method,
                                                                                            mean_method = mean_method)
    crossffiting_two_fold[[split_index]]$influence_function_value <- evaluate_influence_function(crossffiting_two_fold[[split_index]]$sample_1_crossfitting,
                                                                                               crossffiting_two_fold[[split_index]]$sample_2_crossfitting,
                                                                                               crossffiting_two_fold[[split_index]]$nuisance_collection)
    if(!is.null(sample_2)){
      crossffiting_two_fold[[split_index]]$test_statistic <- mean(crossffiting_two_fold[[split_index]]$influence_function_value$inner_product_1) - mean(crossffiting_two_fold[[split_index]]$influence_function_value$inner_product_2)
      #debias
      crossffiting_two_fold[[split_index]]$test_statistic <- crossffiting_two_fold[[split_index]]$test_statistic + mean(crossffiting_two_fold[[split_index]]$influence_function_value$influence_eigenvector_each_subject_1) + mean(crossffiting_two_fold[[split_index]]$influence_function_value$influence_eigenvector_each_subject_2)
    }else{
  ...
  test_statistics_before_studentization <- (crossffiting_two_fold[[1]]$test_statistic + same_sign * crossffiting_two_fold[[2]]$test_statistic)/2
  
  
  ####calculate variance of the test statistics
  # nuisance_collection_not_split <- estimate_nuisance_parameter(sample_1, sample_2,
  #                                                              method = pca_method)
  # influence_function_value_not_split <- evaluate_influence_function(sample_1, sample_2, nuisance_collection_not_split)
  # 
  # if(!is.null(sample_2)){
  ...
  if(!is.null(sample_2)){
    ...
    for(cross_fitting_fold in 1:2){
      estimated_variance <- estimated_variance + nrow(sample_1)^(-1) * var(crossffiting_two_fold[[cross_fitting_fold]]$influence_function_value$for_variance_subject_1) + 
                                                 nrow(sample_2)^(-1) * var(crossffiting_two_fold[[cross_fitting_fold]]$influence_function_value$for_variance_subject_2) 
    }
    estimated_variance <- estimated_variance/2
    standard_error <- sqrt(estimated_variance)
    test_statistics <- test_statistics_before_studentization/standard_error
  
    # standard_error <- sqrt(2^(-1)) * sqrt(nrow(sample_1)^(-1)*var(crossffiting_two_fold[[1]]$influence_function_value$influence_each_subject_1)+
    #                                         nrow(sample_1)^(-1)*var(crossffiting_two_fold[[2]]$influence_function_value$influence_each_subject_1)+
    #                                         nrow(sample_2)^(-1)*var(crossffiting_two_fold[[1]]$influence_function_value$influence_each_subject_2)+
    #                                         nrow(sample_2)^(-1)*var(crossffiting_two_fold[[2]]$influence_function_value$influence_each_subject_2))
    # 
  }else{
    standard_error <- sqrt(2^(-1)) * sqrt(nrow(sample_1)^(-1)*var(crossffiting_two_fold[[1]]$influence_function_value$influence_each_subject_1)+
                                            nrow(sample_1)^(-1)*var(crossffiting_two_fold[[2]]$influence_function_value$influence_each_subject_1))
  ...
  return(list(test_statistics = test_statistics,
              standard_error = standard_error,
              test_statistics_before_studentization = test_statistics_before_studentization,
              crossffiting_two_fold = crossffiting_two_fold))
  
}



not_debiased_cross_fitting_pca <- function(sample_1,
                                           sample_2 = NULL,
                                           pca_method = "sparse_pca",
                                           mean_method = "naive"){
  set.seed(1)
...
estimate_nuisance_parameter_lasso <- function(nuisance_sample_1,
                                              ...
                                              pca_method = "sparse_pca",
                                              mean_method = "soft",
                                              mean_threshold_1 = NULL,
                                              mean_threshold_2 = NULL){
  ...
  # if(is.null(mean_threshold_1)){
  #   # mean_threshold_1 <- max(abs(nuisance_sample_1)) * nuisance_sample_size_1^(-1/4)
  #   variance_feature_wise_1 <- apply(nuisance_sample_1, 2, var)
  #   mean_threshold_1 <- variance_feature_wise_1*nuisance_sample_size_1^(-1/4)
  # }
  ...
  # if(is.null(mean_threshold_2)){
  #   # mean_threshold_2 <- max(abs(nuisance_sample_2)) * nuisance_sample_size_2^(-1/4)
  #   variance_feature_wise_2 <- apply(nuisance_sample_2, 2, var)
  #   mean_threshold_2 <- variance_feature_wise_2*nuisance_sample_size_2^(-1/4)
  #   
  # }
  ...
  if(pca_method == "sparse_pca"){
    ...
    estimate_leading_pc <- irlba(sample_centered, nv = 1)$v
    estimate_leading_pc <- estimate_leading_pc/norm(estimate_leading_pc, type = '2')
  }else if(pca_method == "hard"){
    num_latent_factor <- 1
    estimate_leading_pc <- dense_leading_pc <- irlba(sample_centered, nv = num_latent_factor)$v
  ...
  if(mean_method %in% c("lasso", "lasso_no_truncation")){
    ...
    lasso_cv_result <- cv.glmnet(y = nuisance_sample$class,
    ...
    estimate_covariance_matrix_diagonal<- apply(sample_centered, 2, function(x) sum(x^2))/nuisance_sample_size
    estimate_covariance_matrix_trace <- sum(estimate_covariance_matrix_diagonal)
    estimate_noise_variance <- (estimate_covariance_matrix_trace - estimate_eigenvalue)/(feature_number - 1)
    
    estimate_lasso_beta <- colMeans(nuisance_sample_1) - colMeans(nuisance_sample_2)
    
    ####thresholding it
    
    variance_feature_wise_centered <- apply(sample_centered, 2, var)
    beta_threshold <- effective_sample_size^(-1/4) * 2*sqrt(variance_feature_wise_centered)*sqrt(2*log(2*feature_number*effective_sample_size))
    # beta_threshold <- 2*sqrt(variance_feature_wise_centered)*sqrt(2*log(2*feature_number*effective_sample_size)/effective_sample_size)
    
    # beta_threshold <-  (min(nuisance_sample_size_1,nuisance_sample_size_2))^(-1/4)
    if(mean_method == "hard"){
      #hard-thresholding
      estimate_lasso_beta[abs(estimate_lasso_beta) < beta_threshold] <- 0
    }else if(mean_method == "soft"){
      #soft-thresholding
      estimate_lasso_beta <- sign(estimate_lasso_beta) * pmax(abs(estimate_lasso_beta) - beta_threshold, 0)
    }
    
    # estimate_lasso_beta <- solve(simulation_covariance) %*% estimate_lasso_beta[1:number_feature]
    # if(norm(estimate_lasso_beta, '2') > 0){
    #   estimate_lasso_beta <- estimate_lasso_beta/norm(estimate_lasso_beta, '2') 
    # }
    estimate_optimal_direction <- (estimate_noise_variance)^(-1) * (estimate_lasso_beta - (1-estimate_noise_variance)^(-1) * as.numeric(crossprod(estimate_leading_pc, estimate_lasso_beta)) * estimate_leading_pc)
    
    #####
    # pc_beta_inner_product <- crossprod(estimate_leading_pc, estimate_lasso_beta)
    # estimate_projection_direction <- estimate_leading_pc + 3 * as.numeric(sign(pc_beta_inner_product)) * estimate_lasso_beta
    if(norm(estimate_optimal_direction, type = '2') > (effective_sample_size)^(-1/4)){
      print('bingo')
      # estimate_optimal_direction <- sign(estimate_optimal_direction) * pmax(abs(estimate_optimal_direction) - (effective_sample_size)^(-1/3),0)
      estimate_projection_direction <- (effective_sample_size)^(-1/2)*estimate_leading_pc + (effective_sample_size^(1/2) - 1)* (effective_sample_size)^(-1/2)* estimate_optimal_direction
    }else{
...
#not_debiased_cross_fitting_pca_lasso_multiple_split
...
# 
# for(i in 1:15){
#   olds[i] <- not_debiased_cross_fitting_pca_lasso(sample_1, sample_2,
#                                                   pca_method = pca_method)$test_statistics
#   news[i] <- not_debiased_cross_fitting_pca_lasso_multiple_split(sample_1, sample_2, 
#                                                                  pca_method = pca_method, n_folds = 10)$test_statistics
# }
# 
# hist(olds)
# 
# hist(news)
# plot(abs(olds), abs(news))
# abline(0,1)
...
# debug(debiased_cross_fitting_pca)
# cross_fitting_result <- debiased_cross_fitting_pca(sample, sample)
# undebug(debiased_cross_fitting_pca)
# cross_fitting_result$test_statistics
# cross_fitting_result$standard_error

# inner_product_and_studentization <- function(cross_fitting_sample_1,
#                                              cross_fitting_sample_2 = NULL,
#                                              probe_vector){
#   
#   cross_fitting_sample_1 <-  as.matrix(cross_fitting_sample_1)
#   
#   crossfitting_each_subject_sample_1 <- cross_fitting_sample_1 %*% probe_vector
#   crossfitting_mean_sample_1 <- mean(crossfitting_each_subject_sample_1)
#   crossfitting_sd_sample_1 <- sd(crossfitting_each_subject_sample_1)
#   
#   if(!is.null(cross_fitting_sample_2)){
#     print("performing two-sample mean testing")
#     cross_fitting_sample_2 <-  as.matrix(cross_fitting_sample_2)
#     
#     crossfitting_each_subject_sample_2 <- cross_fitting_sample_2 %*% probe_vector
#     crossfitting_mean_sample_2 <- mean(crossfitting_each_subject_sample_2)
#     crossfitting_sd_sample_2 <- sd(crossfitting_each_subject_sample_2)
#   }
#   
#   if(is.null(cross_fitting_sample_2)){
#     sample_1_size <- nrow(cross_fitting_sample_1)
#     standard_error <- crossfitting_sd_sample_1/sqrt(sample_1_size)
#     test_statistics <- crossfitting_mean_sample_1/standard_error
#     
#     return(list(test_statistics = test_statistics,
#                 standard_error = standard_error,
#                 probe_vector = probe_vector,
#                 crossfitting_each_subject_sample_1 = crossfitting_each_subject_sample_1,
#                 crossfitting_each_subject_sample_2 = NULL))
#   }else{
#     sample_1_size <- nrow(cross_fitting_sample_1)
#     sample_2_size <- nrow(cross_fitting_sample_2)
#     
#     standard_error <- sqrt(crossfitting_sd_sample_1^2/sample_1_size + crossfitting_sd_sample_2^2/sample_2_size)
#     test_statistics <- (crossfitting_mean_sample_1 - crossfitting_mean_sample_2)/standard_error
#     
#     return(list(test_statistics = test_statistics,
#                 standard_error = standard_error,
#                 probe_vector = probe_vector,
#                 crossfitting_each_subject_sample_1 = crossfitting_each_subject_sample_1,
#                 crossfitting_each_subject_sample_2 = crossfitting_each_subject_sample_2))
#   }
# }
# 
# 
# # debug(inner_product_and_studentization)
# # test_result <- inner_product_and_studentization(cross_fitting_sample, probe_vector = estimated_pc)
# # undebug(inner_product_and_studentization)
# # test_result$test_statistics
# # test_result$standard_error
