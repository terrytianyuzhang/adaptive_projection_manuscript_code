...
Part of this work was completed while T.Z. was at Carnegie Mellon University. The authors gratefully acknowledge Jin-Hong Du for assistance with the preparation of the data presented in \Cref{section: real data}.

\section*{Conflict of Interest}
The authors declare that they have no conflicts of interest.

\section*{Funding}
 This project was funded by National Institute of Mental Health grant R01MH123184 and NSF DMS-2015492.
...
\begin{figure}[!t]
    \centering     \includegraphics[width =0.7\linewidth]{paper_plot/null_plot.pdf}
...
\begin{revblock}
In this section, we use a simplified example to illustrate the intuition of one-step correction in statistics. Suppose we are interested in estimating the following quantity $\gamma = \mu_X^\top v_1$, where $\mu_X$ is the population mean and $v_1$ is the leading PC direction. If $v_1$ is known and the mean is estimated, $\mu_X^{(m)\top} v_1$ is close to a Gaussian distribution, which greatly facilitates inference. Consider the plug-in estimator $ \gamma_{\rm pi}^{(-m)} = \mu_X^{(m) \top} v_1^{(-m)}$.
...
% To further simplify the presentation, we will consider a one-sample version of $\theta$ to illustrate the aforementioned principle. The extension to the corresponding two-sample problem is straightforward. Consider $\gamma = \mu_X^\top v_1$ and a plug-in estimator $\hat \gamma_{\rm pi}^{(-m)} = \mu_X^{(m) \top} v_1^{(-m)}$.

One may attempt to use the following steps to characterize the asymptotic behavior of $\gamma_{\rm pi}^{(-m)}$:
...
\end{equation}
The first term in the RHS of \Cref{eq: example decomposition} converges to a normal distribution after proper normalization, which we will refer to as the CLT term. The second term is often of a higher order than the CLT term because it is a product of two terms both shrinking to zero.
The third term usually prevents $\gamma_{\rm pi}^{(-m)}$ from establishing a Gaussian distribution in high-dimensional settings. It corresponds to the bias due to using the estimated $v_1^{(-m)}$. When the dimension of $X$ is large, $v_1^{(-m)}$ often converges to $v_1$ at a slower rate than the CLT term, and its irregular distribution---possibly due to the induced sparsity---dominates the CLT term.  

A closer examination of this problematic term enlightens a solution. For a fixed $\mu_X$, we can treat $\mu_X^\top v_1^{(-m)}$ as a mapping from the distribution associated with $\mathcal{D}^{(-m)}$, denoted as $P_n^{(-m)}$, to a number. So the third term in \eqref{eq: example decomposition} can be rewritten as
\begin{equation*}
\mu_X^{\top}v_1^{(-m)}-\mu_X^{\top}v_1 =: f_\gamma(P_n^{(-m)}) - f_\gamma(P),
\end{equation*}
where we use $P$ to denote the law of $X$. The function $f_{\gamma}(\cdot)$ takes in a distribution and returns the inner product, which can be quite opaque. However, the von Mises expansion states that for regular $f_{\gamma}$'s, we can linearize this difference:
\begin{equation}\label{eq: example_von_mises}
   f_\gamma(P_n^{(-m)}) - f_\gamma(P) = -E_P[\varphi_\gamma(X;P_n^{(-m)})] + \text{higher order remainder }
\end{equation}
with a function $\varphi_\gamma$ that can be explicitly calculated---which is known as the influence function of the parameter $\gamma$. So now we know the third term in \eqref{eq: example decomposition} is essentially an expectation plus a statistically smaller term. (See \cite{fisher2021visually} for more intuition and visual illustrations on the expansion.)
\end{revblock}
...
\label{th: one step}
    ...
    \begin{itemize}
         ...
         \end{equation}
         Similar conditions also hold for $\|\mu_X-\mu_X^{(-m)}\| $ and $\|\mu_Z-\mu_Z^{(-m)}\|$.
    \end{itemize}
...
\begin{revblock}
In our case, we can apply some regularized estimators of $\mu_X, \mu_Z$ to achieve the $o_P(n^{-1/4})$ rate. One choice is simply calculating simple sample means from $\mathcal{D}^{(-m)}$ and hard thresholding each entry at $\sqrt{\log p/n}$. This procedure and its close variations give estimators converging at rate $\sqrt{\log p/n} \ll n^{-1/4}$, assuming a small number of entries of $\mu_X$ are non-zero or it lies in some $l_q$-balls with $0<q<1$ \cite{donoho1994, johnstone2019gaussian}.
...
Estimating high-dimensional covariance matrices is comparatively more recent. This task is possible when the covariance matrix satisfies certain structures, including low-rank, approximate block-diagonal, or sparsity. The theoretical rates of many estimators, measured in the operator spectral norm $\|\Sigma^{(-m)} - \Sigma\|$, are often of order $\sqrt{\log p/n}$ or $n^{-\alpha/(2\alpha + 1)}$ with some regularity index $\alpha > 0$, possibly achieving the required $o(n^{-1/4})$ rate in \eqref{eq: quarter rate}. For light-tail distributions, computing sample covariance matrix and thresholding small elements to zero can cover a wide range of sparse truth (also illustrated in our \Cref{app: additional simulation}). Modifying the sample covariance matrix to some robust versions, the thresholding pipeline can achieve the $o_P(n^{-1/4})$ rate even when data is much more dispersed \cite{avella2018robust}.
% We also refer our readers to (e.g., \cite{fan2016overview}) for more extensive surveys.
\end{revblock}
...
\begin{figure}[!t]
    \centering
    \includegraphics[width =0.9\linewidth]{paper_plot/24_main_simulation.pdf}
...
\begin{revblock}
    The presented results are simulated under spike models. In \Cref{app: additional simulation}, we provide additional results under unstructured sparse covariance matrices.
\end{revblock}
...
The number of active genes for each of the PC1-PC4 is approximately $20$ (\Cref{fig: upset}). PC1 ($p=0.047$) includes $18$ genes and $14$ of them are reported to be marginally significant according to a standard univariate negative-binomial regression (p-value threshold $0.05$); however, only one survives adjustment for multiple comparisons. A further inspection of the PC1 active genes and the estimated $v_1^{(-m)}$ is provided in \Cref{fig: interpretation} A, C. 
% These genes are all highly correlated and likely have similar functions in the immune system.

\begin{revblock}
    

The PC4 support genes possess a more interpretable pattern. Its active genes include IFI27, IFI44L, IFI6, IFIT3, ISG15, and MX1. As their names suggest (the ``IFI" prefix stands for "InterFeron-Inducible"), SLE cases and healthy controls differ significantly in the interferon-related pathway. Of the 19 PC4 support genes, eight are also identified as differentially expressed by the multiplicity-adjusted single-gene tests. Although the two analyses thus provide complementary descriptions of the same signal, the proposed framework also offers evidence that these eight gene-level findings are not isolated or accidental. Their expression levels are correlated, and their between-group differences can be interpreted jointly as one unit. (Further information regarding the PC4 genes is provided in \Cref{appfig: inspect PC4}.)

\end{revblock}

% PC4 ($p=0.004$) includes 19 genes and $9$ of them are reported marginally significant according to the marginal tests ($0.05$-level) and 8 retain significance after multiple comparison corrections. We provide the gene names and their correlation in \Cref{appfig: inspect PC4}  In panel A, we can observe the $19$ genes are divided into two association blocks: One contains all the mitochondrial genes, which are not protein-coding genes.  These genes are not well studied in the literature and are often removed from such analyses. The other block contains multiple genes having significant functions in the immune system: as many of their names suggest, the ``IFI" prefix stands for "InterFeron-Inducible," indicating that these genes are up-regulated in response to interferon signaling, which is an anti-virus mechanism in the human body.
...
\section{More Details on the Debiased Test}

\subsection{Explicit formula}\label{app: explicit formula}
...
\begin{revblock}
\subsection{A Self-contained Description of the Debiased Test Pipeline}

\noindent
Input: samples $\{X_i\}_{i=1}^{N_X}$ and $\{Z_i\}_{i=1}^{N_Z}$,
number of folds $M$, and significance level $\alpha$.

\begin{enumerate}
\item Randomly partition each group into $M$ equal-sized folds,
with $n_X=N_X/M$ and $n_Z=N_Z/M$ observations per fold.
Let $\mathcal D^{(m)}$ denote fold $m$ and
$\mathcal D^{(-m)}=\mathcal D\setminus\mathcal D^{(m)}$.

\item For each $m=1,\ldots,M$:
\begin{enumerate}
\item Using only $\mathcal D^{(-m)}$, estimate
$\mu_X^{(-m)}$, $\mu_Z^{(-m)}$, and $\Sigma^{(-m)}$,
and obtain the leading sparse PC $v_1^{(-m)}$ and
eigenvalue estimate $\lambda_1^{(-m)}$. Use a common sign convention for the PC estimates.
...
s^{(-m)\top}
\left[
(g-\mu_G^{(-m)})(g-\mu_G^{(-m)})^\top
-\Sigma^{(-m)}
\right]v_1^{(-m)}.
\]

\item Using the held-out fold $\mathcal D^{(m)}$, compute
the sample means $\mu_X^{(m)}$ and $\mu_Z^{(m)}$ and
the corrected projected difference
\[
\hat\theta_{\rm 1s}^{(m)}
=
...
\item Set $w=n_X/(n_X+n_Z)$ and compute the held-out scores
\[
\begin{aligned}
V_X^{(-m)}(X_i)
&=(X_i-\mu_X^{(-m)})^\top v_1^{(-m)}
...
\end{aligned}
\]
Let $\bar V_G^{(m)}$ be the average score for group $G$
in fold $m$, and calculate
\[
\hat q_G^{(m)}
=
\frac{1}{n_G}
\sum_{G_i\in\mathcal D^{(m)}}
\left(V_G^{(-m)}(G_i)-\bar V_G^{(m)}\right)^2,
\qquad G\in\{X,Z\}.
\]
\end{enumerate}

\item Aggregate the fold estimates:
\begin{equation*}
    T_{\rm 1s}
 =
\frac{\sum_{m=1}^M\hat\theta_{\rm 1s}^{(m)}}
{\hat\sigma_{\rm 1s}} 
\end{equation*}
where
\begin{equation*}
 ...
 =
\sum_{m=1}^M
\left(\frac{\hat q_X^{(m)}}{n_X}
+\frac{\hat q_Z^{(m)}}{n_Z}\right)
\end{equation*}

\item Return the two-sided $p$-value
\[
p_{\rm 1s}=2\left\{1-\Phi\left(|T_{\rm 1s}|\right)\right\},
\]
where $\Phi$ is the standard normal distribution function.
Reject $H_0^{\rm proj}(v_1)$ if $p_{\rm 1s}<\alpha$.
\end{enumerate}
\end{revblock}
...
\end{aligned}
\end{equation}
 are all of order $o_P(n^{-1/2})$. We present the details of the argument in \Cref{lemma: bound terms in remainder}. Combining this result with \eqref{eq: simple X term} and \eqref{eq: simple Z term}, we conclude our proof.
\end{proof}
...
\section{Simulation Under Unstructured Sparse Correlation}
\label{app: additional simulation}

\begin{revblock}

The simulation data in \Cref{section: simulation} is generated under spike models where the population eigenvectors are sparse. To illustrate that the proposed methods are applicable beyond these settings, we conducted supplementary simulation studies. In this section, we consider high-dimensional unstructured covariance matrices where the eigenvectors have dense supports over all the dimensions.

\subsection{Unstructured Covariance Matrix}
We considered two settings where $p\in\{100,1000\}$. The covariance matrix was determined as follows: We first created a symmetric matrix $A$
with zero diagonal elements. We then selected $4p$ unordered off-diagonal pairs $\{i,j\}$ uniformly at
random and assigned each selected $A_{ij} = A_{ji}$ a common random value. After obtaining $A$, the population covariance matrix was
\[
\Sigma=I_p+0.9\frac{A}{\lVert A\rVert_{\rm op}}.
\]
We iterated over several $A$'s and retained the first generated covariance matrix whose leading eigengap was at
least $0.2$. For each dimension $p$, we fixed this covariance matrix across all repetitions and sample sizes.

Since we are altering, randomly, only $\sim p$ of the total $\sim p^2$ off-diagonal to non-zero numbers, such matrices are estimable in high-dimensional settings. This is because these matrices do not accumulate too many non-zero elements in each row or column. For both dimensions, the covariance matrices generated in this fashion typically have $8$ non-zero elements for each row. Both of them have a maximum of $17$ non-zero elements in a single row. Such matrices are sparse in a nicer way: in \cite{bickel2008covariance}, Theorem~1, the authors showed that hard-thresholding is effective for estimating this type of sparse matrices (our matrices lie in their $\mathcal{U}_\tau\left(0, c_0(p), M\right)$ class). Note that these matrices have dense eigenvectors.

\subsection{Mean Shifts under Projected Null and Alternative}

Let $S$ contain the ten coordinates with the largest absolute entries of
$v_1$. We constructed a unit vector $\delta_\perp$, supported on $S$, by
pairing these coordinates and swapping their $v_1$ loadings with alternating
signs. This construction gives $\delta_\perp^\top v_1=0$. Under the projected
null, the population mean difference was $5\delta_\perp$ for both dimensions.
Consequently, $\mu_X$ and $\mu_Z$ differ substantially while
still satisfying
\[
(\mu_X-\mu_Z)^\top v_1=0.
\]
For the alternative, we used
\[
\mu_X-\mu_Z=
\begin{cases}
\displaystyle
\delta_\perp+0.25\cdot \frac{v_{1,S}}{v_1^\top v_{1,S}}, & p=100,\\[8pt]
\displaystyle
\delta_\perp+2\cdot \frac{v_{1,S}}{v_1^\top v_{1,S}}, & p=1000,
\end{cases}
\]
where $v_{1,S}$ agrees with $v_1$ on $S$ and is zero elsewhere. We used a larger signal size ($2$) for $p=1000$ than $p = 100$ ($0.25$) to illustrate a more interesting power regime (which also reflects that higher dimensions make the problems harder). After determining the mean shifts, we used them to define the true mean for each group $\mu_X=(\mu_X-\mu_Z)/2$ and $\mu_Z=-(\mu_X-\mu_Z)/2$. The observations are normal vectors based on $(\mu_X, \Sigma)$ and $(\mu_Z, \Sigma)$. We used balanced sample sizes for both groups.

\subsection{Implementation of the Plug-in and Debiased Methods}

We implemented both methods using five-fold cross-fitting. When estimating the covariance matrix, we first compute the simple sample covariance matrix pooling both control and treatment samples. Afterwards, we obtain the hard-thresholded estimator $\Sigma^{(-m)}$ by truncating off-diagonal elements smaller than $\sim \sqrt{\log p / (N_X + N_Z)}$ to zero. The other nuisance including $\widehat{\lambda}_1^{(-m)}$ and $\widehat{v}_1^{(-m)}$ are directly computed using the matrix estimator. Under the row-wise sparsity and eigen-gap, the theoretical estimation errors of the nuisance are of order $\sqrt{\log p / (N_X + N_Z)}$, smaller than the requirement in \Cref{th: one step}.

\begin{figure}[!tbp]
    \centering
    \includegraphics[width=\linewidth]{paper_plot/037_unstructured_rejection_rate.pdf}
    \caption{Rejection rates in the unstructured sparse covariance simulation. The plug-in method rejects in all settings, while the debiased proposal has well-controlled size under the projected null. Error bars are $95\%$ Monte Carlo intervals.}
    \label{appfig: unstructured sparse covariance simulation}
\end{figure}

\subsection{Simulation Results}

We report the rejection rates at $p<0.05$ of two methods in \Cref{appfig: unstructured sparse covariance simulation}. The plug-in method always has a large rejection rate, regardless of whether it is under the projected null or the genuine alternative. It is not suitable to test the projected null where the signal is orthogonal to $v_1$ (left column). However, the debiased method has the desired property: it is well-calibrated under the projected null, which can be used to answer the question of whether the signal aligns with the eigenvector direction. Under the alternative, it also has competitive power. Although the plug-in method rejects much more often in the ($p = 1000$, alternative) setting, it can only control type-I error when the means are exactly identical.
