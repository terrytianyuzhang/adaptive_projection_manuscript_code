...
The PC4 support genes possess a more interpretable pattern. Its active genes include IFI27, IFI44L, IFI6, IFIT3, ISG15, and MX1. As their name suggest (the ``IFI" prefix stands for "InterFeron-Inducible"), SLE cases and healthy controls differ significantly in the interferon-related pathway. Of the 19 PC4 support genes, eight are also identified as differentially expressed by the multiplicity-adjusted single-gene tests. Although the two analysis thus provide complementary descriptions of the same signal, the proposed framework also offers evidence that these eight gene-level findings are not isolated or accidental. Their expression levels are correlated, and their between-group differences can be interpreted jointly as one unit. (Further information regarding PC4 genes are provided in \Cref{appfig: inspect PC4}.)
