...
The PC4 support genes possess a more interpretable pattern. Its active genes include IFI27, IFI44L, IFI6, IFIT3, ISG15, and MX1. As their name suggest---the ``IFI" prefix stands for "InterFeron-Inducible". A strong signal aligned with this PC ($p = 0.004$) implies a significant change in the interferon-related pathway. When we examine each of the 19 active genes individually, eight of them are significantly differentially expressed according to single-gene tests. Although the PC-level test and the single-gene level test agree with each other well, the proposed framework provides additional information that the eight significant genes are not accidental; the data set supports that they have correlated expression levels and their difference between the groups should be treated as one unity. We provide the gene names and their correlation for PC4 in \Cref{appfig: inspect PC4}

\end{revblock}

% consistent with the interferon-related transcriptional signature previously documented in lupus immune cells
...
random and assigned each selected $A_{ij} = A_{ji}$ a common random value. After obtaning $A$, the population covariance matrix was
\[
...
Consequently, the $\mu_X \neq \mu_Z$ substantially while
still satisfying
\[
(\mu_X-\mu_Z)^\top v_1=0.
\]
